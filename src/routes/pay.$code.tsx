...
function PayPage() {
  ...
  if (link === null) {
    return <Shell><Empty title="Link not found" body="This payment link doesn't exist or has been removed." /></Shell>;
  }
  if (link.effective_status === "paid") {
    return <Shell><Empty title="Already paid" body="This payment has already been completed. Thank you." /></Shell>;
  }
  if (link.effective_status === "expired") {
    return <Shell><Empty title="Link expired" body="Please contact us for a fresh payment link." /></Shell>;
  }
  if (link.effective_status === "cancelled") {
    return <Shell><Empty title="Link cancelled" body="This payment request was cancelled." /></Shell>;
  }
  if (link.effective_status === "draft") {
    return <Shell><Empty title="Not available" body="This link isn't active yet." /></Shell>;
  }
    ...
    <Shell brand={link.brand_name} tagline={link.brand_tagline}>
      <div className="glass rounded-3xl p-6 shadow-card sm:p-8">
        ...
        <div className="mt-1 text-sm text-muted-foreground">
          For {link.client_name}{link.invoice_ref ? ` · Invoice #${link.invoice_ref}` : ""}
        </div>
        {link.description && (
        ...
        <div className="mt-6 rounded-2xl border border-border bg-background/40 p-5">
          ...
          <div className="mt-1 text-xs text-muted-foreground">Charged in {currency}. No INR amount is shown.</div>
        </div>

        {link.allow_tip && (
          <div className="mt-6">
            <div className="text-sm font-medium">
              Add a tip for the team <span className="font-normal text-muted-foreground">(optional)</span>
            </div>
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              {link.tip_presets.map((preset) => (
                ...
                >
                  {preset}% · {formatMoney(Math.round((link.base_amount_minor * preset) / 100), currency)}
                </TipChip>
              ))}
            ...
            {tipMode === "custom" && (
              <div className="mt-3">
                ...
                {customTipInvalid && (
                  <p className="mt-2 text-xs text-destructive">Enter a tip within the permitted range.</p>
                )}
              </div>
            )}
          </div>
