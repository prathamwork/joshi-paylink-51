    ...
    <AdminShell title="New payment link">
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="glass space-y-5 p-6 lg:col-span-2">
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Client name *">
              <Input value={form.client_name} onChange={(event) => setForm({ ...form, client_name: event.target.value })} maxLength={120} required />
            </Field>
            <Field label="Project / service *">
              <Input value={form.project_title} onChange={(event) => setForm({ ...form, project_title: event.target.value })} maxLength={160} required />
            </Field>
            ...
            <Field label="Client country">
              <Input value={form.client_country} onChange={(event) => setForm({ ...form, client_country: event.target.value })} />
            </Field>
            <Field label="Invoice reference">
              <Input value={form.invoice_ref} onChange={(event) => setForm({ ...form, invoice_ref: event.target.value })} />
            </Field>
          </div>

          <Field label="Description">
            <Textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} maxLength={2000} rows={3} />
          </Field>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <Field label="Currency">
              <Select value={form.currency} onValueChange={(value) => setForm({ ...form, currency: value as CurrencyCode })}>
                ...
                <SelectContent>
                  {enabled.filter((currency): currency is CurrencyCode => currency in CURRENCIES).map((currency) => (
                    <SelectItem key={currency} value={currency}>
                      {currency} — {CURRENCIES[currency].label}{CURRENCIES[currency].requiresIntl ? " ⚠️" : ""}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          {CURRENCIES[form.currency]?.requiresIntl && (
            <p className="text-xs text-warning">
              ⚠️ Cashfree must approve International Payment Gateway and enable {form.currency} on your merchant account before live use.
            </p>
          )}

          <div className="flex flex-wrap items-center gap-4">
            <label className="flex items-center gap-2 text-sm">
              <Switch checked={form.allow_tip} onCheckedChange={(value) => setForm({ ...form, allow_tip: value })} /> Allow tips
            </label>
            <label className="flex items-center gap-2 text-sm">
              <Switch checked={form.tip_custom_allowed} onCheckedChange={(value) => setForm({ ...form, tip_custom_allowed: value })} /> Allow custom tip
            </label>
            <label className="flex items-center gap-2 text-sm">
              <Switch checked={form.single_use} onCheckedChange={(value) => setForm({ ...form, single_use: value })} /> Single-use
            </label>
          </div>
          ...
          <Button
            ...
            disabled={mutation.isPending || !form.client_name || !form.project_title || !form.amount || amountMinor <= 0}
        ...
        <Card className="glass h-fit p-6 lg:sticky lg:top-6">
          <div className="text-xs uppercase tracking-wide text-muted-foreground">Live customer preview</div>
          <div className="mt-4">
            ...
            {form.description && <p className="mt-3 whitespace-pre-line text-sm text-muted-foreground">{form.description}</p>}
            <div className="mt-6 text-3xl font-bold">{amountMinor > 0 ? formatMoney(amountMinor, form.currency) : "—"}</div>
            <div className="mt-1 text-xs text-muted-foreground">The customer sees {form.currency}; no INR settlement amount is shown.</div>
            {form.allow_tip && (
